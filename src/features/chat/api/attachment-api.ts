...
import { parseMediaVerificationStatus } from "@/lib/verification-status";
import type { ChatAttachment } from "../types";import { chatRoutes } from "./routes";
...
export async function addAttachment(
  ...
  const payload: AddAttachmentBody = {
    ...
    attachment: {
      ...
      verificationStatus: attachment.verificationStatus ?? "pending",
    },
  };
  ...
  return {
    ...
    verificationStatus: parseMediaVerificationStatus(
      body.verificationStatus ?? attachment.verificationStatus,
      "pending",
    ),
  };
}